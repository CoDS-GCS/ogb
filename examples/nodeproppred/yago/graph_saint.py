...
sys.path.insert(1,'/shared_mnt/ogb_original/ogb/nodeproppred/')
...
class RGCN(torch.nn.Module):
    ...
    def inference(self, x_dict, edge_index_dict, key2int):
        ...
        for key, (row, col) in edge_index_dict.items():
            if key ==('paper','cites','paper') and not (paper_count in row):
                row=torch.cat((row, torch.tensor([paper_count])))
                col=torch.cat((col, torch.tensor([paper_count-1])))
