...
def main():
    ...
    for i, covrage_row in fieldOfStudy_Coverage_df.iterrows():
        ...
        for sample_key in sampledQueries:
            if i >= 0:
                ...
                if str(covrage_row["topic"])=="FM" :
                    ...
                    # dataset_name = "OBGN_MAG_Usecase_" + str(int(covrage_row["Q_idx"])) + "_" + str(str(covrage_row["topic"]).strip().replace(" ", "_").replace("/", "_"))
                    dataset_name = "OBGN_MAG_" + sample_key + "Usecase_" + str(int(covrage_row["Q_idx"])) + "_" + str(str(covrage_row["topic"]).strip().replace(" ", "_").replace("/", "_"))
                print("dataset_name=",dataset_name)

                dic_results[dataset_name] = {}
                dic_results[dataset_name]["q_idx"] = int(covrage_row["Q_idx"])
                ...
                dataset = PygNodePropPredDataset_hsh(name=dataset_name,root = '/media/hussein/UbuntuData/OGBN_Datasets/Sparql_Sampling/')
                ...
                if args.loadTrainedModel==1:
                    ...
                    pd.DataFrame(dic_results).transpose().to_csv("/media/hussein/UbuntuData/OGBN_Datasets/Sparql_Sampling/OGBN_MAG_RGCN_times" + ".csv", index=False)
                    shutil.rmtree("/media/hussein/UbuntuData/OGBN_Datasets/Sparql_Sampling/" + dataset_name)
                    torch.save(model.state_dict(), "/media/hussein/UbuntuData/OGBN_Datasets/Sparql_Sampling/"+dataset_name+"_RGCN_QM.model")
## Calc F1 Score
