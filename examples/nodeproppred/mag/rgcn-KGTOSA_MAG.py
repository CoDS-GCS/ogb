...
def main():
    ...
    #         try:
    #             dataset = PygNodePropPredDataset_hsh(name=dataset_name, root='/shared_mnt/DBLP/',
    #                                                  numofClasses='50')
    #         except:
    #             print('can\'t load FM')
    ...
    rgcn_start_t = datetime.datetime.now()
    start_t = datetime.datetime.now()
    dataset_name = "KGTOSA_MAG_Paper_Venue_StarQuery_Discipline"
    # dataset_name = "KGTOSA_MAG_StarQuery_Discipline"
    # dataset_name ="ogbn-mag-ReleventSubGraph_Discipline"
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            # print_memory_usage()
            print("data.y_dict[subject_node]=", max(data.y_dict[subject_node]), min(data.y_dict[subject_node]),
                  len(data.y_dict[subject_node]))
