...
edge_index_dict[('institution', 'inv_affiliated_with', 'author')] = torch.stack([c, r])

r, c = edge_index_dict[('author', 'writes', 'paper')]
edge_index_dict[('paper', 'inv_writes', 'author')] = torch.stack([c, r])

r, c = edge_index_dict[('paper', 'has_topic', 'field_of_study')]
edge_index_dict[('field_of_study', 'inv_has_topic', 'paper')] = torch.stack([c, r])
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        print(DecodedSubgraph_df[DecodedSubgraph_df["Src_Node_Type"].isin(["paper"])])
        DecodedSubgraph_df.to_csv("OGB_MAG_DecodedSubgraph_"+str(Subgraph_idx)+".csv",index=None)
        data = data.to(device)
