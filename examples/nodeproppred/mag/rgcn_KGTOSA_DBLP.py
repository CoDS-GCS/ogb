...
def main():
    ...
    gsaint_Final_Test=0
    for dataset_name in DBLP_datasets:    
        rgcn_start_t = datetime.datetime.now()
        start_t = datetime.datetime.now()
        dataset = PygNodePropPredDataset_hsh(name=dataset_name,root='/shared_mnt/DBLP/KGTOSA_DBLP_Datasets/',numofClasses='50')
        ...
        for keys, (row, col) in data.edge_index_dict.items():
            if (keys[1] in remove_pedicates):
                to_remove_rels.append(keys)
                to_remove_rels.append((keys[2], '_inv_', keys[0]))
        ...
        for keys, (row, col) in data.edge_index_dict.items():
            ...
            if keys[0] != keys[2]:  ## subject and object are diffrent
                ...
                data.adj_t_dict[(keys[2], '_inv_', keys[0])] = adj
            else:
        ...
        model = RGCN(feat.size(-1), args.hidden_channels,
                     ...
                     data.num_nodes_dict, subject_node, edge_types)
        ...
        pd.DataFrame(dic_results).transpose().to_csv("/shared_mnt/DBLP/KGTOSA_DBLP_Datasets/"+dataset_name+ "_RGCN_times.csv",
            index=False)
