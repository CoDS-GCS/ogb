...
def graphSaint():
    ...
    def train(epoch,org_dataset=None):
        ...
        for data in train_loader:
            ...
            DecodedSubgraph_df.to_csv("DBLP_GA_GS_DecodedSubgraphs/DBLP_GA_GS_DecodedSubgraph_" + str(Subgraph_idx) + ".csv", index=None)
    ...
    for GA_dataset_name in MAG_datasets:  
        try:
            ...
            # dir_path = "/shared_mnt/YAGO/" + GA_dataset_name
    ...
    #         ####################
            dataset = PygNodePropPredDataset_hsh(name=GA_dataset_name, root='/shared_mnt/DBLP/KGTOSA_DBLP_Datasets/',numofClasses=str(51))
            ...
            to_keep_rels = [
                'schema#orcid','schema#primaryAffiliation','schema#authoredBy',
                'schema#bibtexType','schema#publishedInSeriesVolume','schema#yearOfEvent']
            ...
            #     # 'db'
            # ]
            to_remove_rels = ['22-rdf-syntax-ns#type',
                                'owl#differentFrom',
                                'rdf-schema#label',
                                'schema#archivedWebpage',
                                'schema#awardWebpage',
                                'schema#creatorNote',
                                'schema#otherAffiliation',
                                'schema#otherFullCreatorName',
                                'schema#otherHomepage',
                                'schema#primaryFullCreatorName',
                                ...
                                'schema#editedBy',
                                'schema#isbn',
                                'schema#listedOnTocPage',
                                'schema#monthOfPublication',
                                'schema#numberOfCreators',
                                'schema#orderedCreators',
                                'schema#otherElectronicEdition',
                                'schema#pagination',
                                'schema#primaryElectronicEdition',
                                'schema#publicationNote',
                                'schema#publishedAsPartOf',
                                'schema#publishedBy',
                                'schema#publishedInJournal',
                                'schema#publishedInJournalVolume',
                                'schema#publishedInJournalVolumeIssue',
                                'schema#publishedInSeries',
                                'schema#title',
                                'schema#wikidata',]
            to_remove_rels_keys = []
            to_keep_subj_object=[]
            for keys, (row, col) in data.edge_index_dict.items():
                if (keys[1] in to_remove_rels):
                    print("to remove keys=",keys)
                    to_remove_rels_keys.append(keys)
                    to_remove_rels_keys.append((keys[2], '_inv_' + keys[1], keys[0]))
                    
            for keys, (row, col) in data.edge_index_dict.items():
                if (keys[0] not in to_keep_subj_object):
                    to_keep_subj_object.append(keys[0])
                if (keys[2] not in to_keep_subj_object):
                    to_keep_subj_object.append(keys[2])
                
            ######################## remove Rels #####################
            for elem in to_remove_rels_keys:
                data.edge_index_dict.pop(elem, None)
                data.edge_reltype.pop(elem,None)
            ######################## remove Nodes #####################
            to_remove_subject_keys=[]
            for key,value in data.num_nodes_dict.items():
                if key not in to_keep_subj_object:
                    to_remove_subject_keys.append(key)
            for key in to_remove_subject_keys:
