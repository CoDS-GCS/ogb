...
class RGCN(torch.nn.Module):
    ...
    def inference(self, x_dict, edge_index_dict, key2int):
        ...
        return x_dict
dic_results = {}
def graphSaint(to_remove_pedicates,to_remove_subject_object,GA_Index,dataset,dataset_name):
    def train(epoch):
        model.train()
    ...
    try:
        ...
        if args.loadTrainedModel == 1:
            ...
            pd.DataFrame(dic_results).transpose().to_csv("/shared_mnt/DBLP/OGBN_DBLP_GSAINT" + dataset_name + ".csv", index=False)
...
DBLP_OGBN_EdgeTypes_df = pd.read_csv(
            "/media/hussein/UbuntuData/GithubRepos/ogb_cods/examples/nodeproppred/mag/DBLP_OGBN_EdgeTypes.csv")
def genetic_algo(data,dataset_name, population_size,features_count, tol_level, top_number):
    ...
    #     print(scores.mean())
    #     return scores.mean()
    def calculate_fitness(to_remove_pedicates, to_remove_subject_object, GA_Index, dataset,dataset_name):
        return graphSaint(to_remove_pedicates, to_remove_subject_object, GA_Index, dataset,dataset_name)

    def get_filtred_edges_nodes(to_remove_edges_idx):
        ...
        return to_remove_pedicates,to_remove_subject_object

    def get_fitness(population,dataset):
        if not hasattr(get_fitness, "counter"):
            get_fitness.GA_Index = 0  # it doesn't exist yet, so initialize it
        get_fitness.GA_Index += 1
        ...
        for individual in population:
            ...
            to_remove_pedicates,to_remove_subject_object=get_filtred_edges_nodes(to_remove_edges_idx)
            individual_fitness = calculate_fitness(to_remove_pedicates, to_remove_subject_object, get_fitness.GA_Index, dataset,dataset_name)
