...
class PygNodePropPredDataset_hsh(InMemoryDataset):
    def __init__(self, name, root = '/media/hussein/UbuntuData/OGBN_Datasets/',numofClasses=349, transform=None, pre_transform=None, meta_dict = None):
        ...
        #TODO split should be random for yago instead of time
        # TODO for yago none of the nodes have features
        #question what are additional node files and additional edge files?
...
# there is this file generated by converter : {...}216_Precipitation\split\time  "nodetype_has_split" which shows which node types are splitted {paper, field of study, author ,...}
        if self.is_hetero:
...
# node label is the class of that node. for example paper 1 is classified as class 5 so it's label is 5
    def process(self):
